;; Paste HTML from the clipboard as Markdown.
;;
;; This file is on the workshop Joyride classpath. workspace_activate calls
;; pastedown/activate! when this window starts.
;; From the workshop folder: npm install --prefix .joyride
;;
;; To use it in every window:
;;   1. Copy this file to ~/.config/joyride/src/pastedown.cljs
;;   2. In ~/.config/joyride: npm install turndown turndown-plugin-gfm
;;   3. In ~/.config/joyride/scripts/user_activate.cljs, require pastedown
;;      and call (pastedown/activate!)
;;
...
(def text-plain-kind (.append vscode/DocumentDropOrPasteEditKind.Text "plain"))

(def pastedown-kind (.append vscode/DocumentDropOrPasteEditKind.Empty "pastedown"))

(defn create-markdown-paste-edits
  "Create plain text and markdown paste edits"
  [dataTransfer]
  (p/let [plain-item (.get dataTransfer "text/plain")
          plain-text (when plain-item (.asString plain-item))
          markdown-content (convert-to-markdown dataTransfer)]
    (let [plain-edit (new vscode/DocumentPasteEdit
                         plain-text
                         "Insert Plain Text"
                         text-plain-kind)
          md-edit (new vscode/DocumentPasteEdit
                       markdown-content
                       "Insert Markdown"
                       pastedown-kind)]
      (set! (.-yieldTo md-edit) #js [vscode/DocumentDropOrPasteEditKind.Text])
      #js [plain-edit md-edit])))
...
(defn create-markdown-paste-provider
  ...
  #js {:provideDocumentPasteEdits
       (fn [_document _ranges dataTransfer context _token]
         (let [paste-as? (= (.-triggerKind context) vscode/DocumentPasteTriggerKind.PasteAs)
               html? (some? (.get dataTransfer "text/html"))]
           (when (or paste-as? html?)
...
(defn register-markdown-paste-provider!
  ...
  (let [provider (create-markdown-paste-provider)
        disposable (vscode/languages.registerDocumentPasteEditProvider
                    "*"
                    provider
                    #js {:providedPasteEditKinds #js [pastedown-kind text-plain-kind]
